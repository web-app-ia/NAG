...
function ApproveExhibitors() {
    ...
    useEffect(() => {
        Axios.get(`http://localhost:8080/api/exhibitions/getByExhibitionOwner/${storedEmail}`)
        .then((response)=>{
        ...
        <Row>
          <Col md="9">
            ...
            <tr key={i}>
              ...
              <td>
                        Name: {exhibitor.name}<br/>
                        Email: {exhibitor.emailAddress}<br/>
                        Contact No: {exhibitor.contactNo}<br/>
                        NIC: {exhibitor.nic}<br/>
                        Company Name: {exhibitor.company}
              </td>
              <td>{exhibitor.enabled? <p></p>:<>
              <button
              style={{ fontSize: "14px", borderRadius: "10px" }}
              className="secondary-button"
              size="sm"
              onClick={()=>handleApproval(exhibitor.emailAddress)}
              >
                Approve
              </button>
              </>}</td>
          ...
          <Card className="card-plain table-plain-bg">
            ...
            <tr key={i}>
              ...
              className="secondary-button"
              size="sm"
              onClick={()=>handleDelete(exhibitor.emailAddress)}
              >
                Delete
              </button>
              </td>
