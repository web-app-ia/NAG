...
export default function GetExhibitions() {
  ...
  // The API returns `started`/`date`; this view was written against legacy
  // `start`/`over`/`datetime` fields — normalize once here.
  const normalizeExhibition = (row) => ({
    ...row,
    start: !!row.started,
    over: false,
    datetime: row.datetime || row.date,
    exhibitionOwner: row.exhibitionOwner || {},
  });

  const handleAdminVisit = (id) => {
    // Staff preview: no ticket required, just count the visit and enter the 3D hall.
    axios
      .put(`http://localhost:8080/api/exhibitions/visitedUsers/${id}`, {}, {
        headers: {
      ...
      .catch(() => {});
    handleClose();
    history.push("/visit-exhibition", { exhibitionId: id });
  };
  ...
  const handleShowExhibition = (id) =>
    axios
      ...
      .then((resOne) => {
        setShowDetails(true);
        setExhibition(normalizeExhibition(resOne.data));
      });
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8080/api/exhibitions", {
        ...
        .then((res) => {
          setExhibitions((res.data || []).map(normalizeExhibition));
          res.data.map((users) => {
                  ...
                  <dl className="d-flex align-items-center">
                    <dl className="row">
                      ...
                      {(localStorage.getItem("userRole") === "ADMIN" ||
                        localStorage.getItem("userRole") === "EX_OWNER") && (
                      <dd className="col-lg-7">
                        <Button
                          style={{ fontSize: "12px", borderRadius: "20px" }}
                          variant="info"
                          size="sm"
                          onClick={() => handleAdminVisit(exhibition.id)}
                        >
                          Visit Exhibition (no ticket required)
                        </Button>
                        <br></br>
                      </dd>)}
                    </dl>
